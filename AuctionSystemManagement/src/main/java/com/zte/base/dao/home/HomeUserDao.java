...
public interface HomeUserDao extends JpaRepository<HomeUser,Long> {
    ...
    @Transactional
    @Modifying
    @Query(value = "update zte_home_user set  status=:#{#homeUser.status} where id=:#{#homeUser.id}",nativeQuery = true)
    ...
    @Transactional
    @Modifying
    @Query(value = "update zte_home_user set balance = :balance where id = :id",nativeQuery = true)
